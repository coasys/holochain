...
///
/// These override the default bootstrap_url, relay_url, and auth material
/// for specific spaces/DNAs.
#[derive(Clone, Deserialize, Serialize, Debug, PartialEq, JsonSchema)]
...
pub struct SpaceNetworkOverride {
    ...
    /// Override the relay server URL for this space. When set, this relay will
    /// be dynamically added to the iroh endpoint when the space is created.
    #[serde(default)]
    #[schemars(schema_with = "holochain_util::jsonschema::optional_url2_schema")]
    pub relay_url: Option<url2::Url2>,
    /// Override the base64-encoded authentication material for the bootstrap
    /// service for this space. Required when the space's bootstrap server
...
pub struct NetworkConfig {
    ...
    /// Per-space network configuration overrides, keyed by DNA hash (base64).
    ///
    /// Allows configuring different bootstrap and relay servers, and per-space
    /// bootstrap auth material, for specific DNA spaces. Overrides specified
    /// here take effect as a fallback when the app manifest does not provide
    /// its own override.
    ///
    /// Example:
    /// ```yaml
  ...
  - driver:
   ...
   "#;
    ...
    network:
      ...
      space_overrides:
        ...
        "uhC0kDnaHash2":
          ...
          relay_url: https://other-relay.tld
        "uhC0kDnaHash3":
    ...
    "#;
        ...
        assert!(override1.base64_auth_material.is_none());
        ...
        assert_eq!(
            override2.relay_url.as_ref().unwrap().as_str(),
            "https://other-relay.tld/"
        );
